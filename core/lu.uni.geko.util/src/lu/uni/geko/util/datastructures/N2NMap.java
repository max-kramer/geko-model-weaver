...
/**
 * TODO MK rewrite the JavaDoc for N2NMap and mention that we do not support
 * removing or changing mapping entries
 *
 * NOTE THAT get(Set<K>) IS NOT SUPPORTED!
 *
 * A {@code Mapping<K,V>} is basically a short way to write
 * {@code Map<K, Set<V>>} but as we do not need all functionality of the Map
 * interface we defined a new interface that only provides the functionality
 * that we needed so far.</br></br> As soon as a full implementation is creation
 * it would be possible to make this interface extend the {@code Map<K, Set<V>>}
 * interface.
 *
 * Whereas we name instances of {@code Map<K, V>} key2ValueMap, instances of
 * {@code Mapping<K,V>} are named key2ValuesMapping (with an s after Value to
 * emphasize that we map on possibly multiple elements).
 *
 * @param <K>
