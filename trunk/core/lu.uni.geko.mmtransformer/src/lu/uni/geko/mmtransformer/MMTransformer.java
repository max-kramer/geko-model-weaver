...
public class MMTransformer extends AbstractModelTransformer<Pair<URI, URI>> {
	...
	/**
	 * Makes the metamodel of the transformer suitable for weaving (if necessary), derives an advice and pointcut
	 * metamodel from it and generates and installs plug-ins with model, edit and editor code for the advice and
	 * pointcut metamodels.
	 *
	 * @return (pointcutMMURI, adviceMMURI)
	 */
	public Pair<URI, URI> generatePcAvMMAndModelCode() {
		EPackage mmPackage = getMMPackage();
		adjustMMIfNecessary(mmPackage);
		URI pcMMURI = generatePcMMAndPlugins(mmPackage, false);
		URI avMMURI = generateAvMMAndPlugins(mmPackage, false);
	...
	private EPackage createPointcutMMPackage(final EPackage mmPackage) {
		String rootElementName = GeKoConstants.getPcMMRootElementName();
	      return createMMPackage(mmPackage, GeKoConstants.getPcMMPkgNameAppendage(), 
	    		  GeKoConstants.getPcMMPkgNsPrefixAppendage(), GeKoConstants.getPcMMPkgNsURIAppendage(), 
	    		  rootElementName);
	}
	...
	private EPackage createAdviceMMPackage(final EPackage mmPackage) {
		String rootElementName = GeKoConstants.getAvMMRootElementName();
	      EPackage adviceMMPackage = createMMPackage(mmPackage, GeKoConstants.getAvMMPkgNameAppendage(), 
	    		  GeKoConstants.getAvMMPkgNsPrefixAppendage(), GeKoConstants.getAvMMPkgNsURIAppendage(), 
	    		  rootElementName);
	 ...
	 * 
	 * @param PcAvMMPkgNameAppendage
	 * 				the string to be appended to the original package name
	 * 
	 * @param PcAvMMPkgNsPrefixAppendage
	 * 				the string to be appended to the original package NsPrefix
	 * 
	 * @param PcAvMMPkgNsURIAppendage
	 * 				the string to be appended to the original package NsURI
	 * 
	 */

	private void setPackageProperties(EPackage AvPcPackage, final String PcAvMMPkgNameAppendage, final String PcAvMMPkgNsPrefixAppendage, final String PcAvMMPkgNsURIAppendage)
	{
		String newName = AvPcPackage.getName() + PcAvMMPkgNameAppendage;
		String newNsPrefix = AvPcPackage.getNsPrefix() + PcAvMMPkgNsPrefixAppendage;
		String newNsURI = AvPcPackage.getNsURI() + PcAvMMPkgNsURIAppendage;

		AvPcPackage.setName(newName);
	 ...
	 */
	private EPackage createMMPackage(final EPackage originalPackage, final String PcAvMMPkgNameAppendage, final String PcAvMMPkgNsPrefixAppendage,
		...
		{
			if(mmContent instanceof EPackage)
			{
				EPackage SubPackage = (EPackage) mmContent;
				setPackageProperties(SubPackage, PcAvMMPkgNameAppendage, PcAvMMPkgNsPrefixAppendage, PcAvMMPkgNsURIAppendage);
			}
		}
