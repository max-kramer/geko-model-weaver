...
/**
 * A class to be used or extended by clients to compare variants of EObjects that is ensuring that values of unordered references
 * are considered equal if they contain the same elements in different order. The implementation is based on
 * {@link ReusableEqualityHelper}.<br/>
 * <br/>
 * <b>Attention</b>: This implementation should be optimised for performance before it is used for comparison of models that
 * contain huge lists as the algorithm runs in asymptotic quadratic time!
 *
...
public class UnorderedReferencesRespectingEqualityHelper extends ReusableEqualityHelper {
   ...
   private boolean equalsIgnoreOrder(final List<EObject> list1, final List<EObject> list2) {
      ...
      // FIXME MK optimise order ignoring list comparison algorithm (runs in asymptotic quadratic time)
